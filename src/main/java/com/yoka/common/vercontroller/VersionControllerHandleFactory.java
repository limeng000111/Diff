 ...
 */
@Component
public class VersionControllerHandleFactory implements CommandLineRunner, ApplicationContext {

    private volatile  ApplicationContext applicationContext;

    private static List<AbstractVersionControl> handler;
    @Override
    public void run(String... args) throws Exception {
        ...
        handler = Lists.newArrayList(checkHandlers);
    }

    public static List<ClassInfoResult> processHandler(VersionController versionController){
        List<ClassInfoResult> result = null;
        for (int i = 0;i < handler.size();i++){
            if (versionController.getCodeManageTypeEnum().equals(handler.get(i).getType())){
                result = VersionControllerHandleFactory.handler.get(i).handler(versionController);
            }
        }
        return result;
    }
